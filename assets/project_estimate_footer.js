(function () {
    ...
    function render(options = {}) {
        ...
        const tabs = estimates.map(estimate => {
            ...
            const isPrimary = Boolean(estimate.is_primary || estimate.isPrimary || estimate.primary);
            ...
            const primaryBadge = isPrimary ? '<span class="est-primary-tag" title="Primary estimate" style="display:inline-flex; align-items:center; gap:3px; background:#f59e0b; color:#0f172a; font-size:0.65rem; font-weight:700; padding:1px 5px; border-radius:3px; margin-left:5px;"><i class="fas fa-star" style="font-size:0.6rem;"></i> Primary</span>' : '';
            const setPrimaryButton = `<button type="button" ${itemActionAttribute}="set-primary" data-estimate-id="${esc(estimateId)}"><i class="fas fa-star" style="color:${isPrimary ? '#f59e0b' : 'inherit'};"></i> ${isPrimary ? 'Primary estimate' : 'Set as primary'}</button>`;
            ...
            return `<span class="est-version-entry"><button type="button" class="est-version-tab${active ? ' active' : ''}" ${selectAttribute}="${esc(estimateId)}" aria-pressed="${active}"><span><strong>${esc(estimate.name || 'Estimate')}${primaryBadge}</strong><small>${esc(estimate.status || 'Draft')} · ${itemLabel}</small></span>${estimate.isLocked ? '<i class="fas fa-lock" aria-label="Locked"></i>' : ''}</button>${menu}</span>`;
        }).join('');
