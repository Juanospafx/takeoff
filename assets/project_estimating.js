(function () {
    ...
    function renderModal() {
        ...
        if (ui.modal === 'new') portal.innerHTML = `<div class="est-dialog" role="dialog" aria-modal="true" aria-labelledby="copyEstimateTitle"><header><h2 id="copyEstimateTitle">New Estimate</h2><button type="button" data-close-modal>&times;</button></header><label>Name<input id="copyEstimateName" value="${esc(current().name)} Copy"></label><fieldset><legend>Starting point</legend><label><input type="radio" name="copyEstimateMode" value="all" checked> Copy everything</label><label><input type="radio" name="copyEstimateMode" value="structure"> Groups only</label><label><input type="radio" name="copyEstimateMode" value="blank"> Blank</label></fieldset><footer><button type="button" data-close-modal>Cancel</button><button type="button" class="est-btn-primary" data-create-estimate data-est-action="create-estimate-copy">Create</button></footer></div>`;
