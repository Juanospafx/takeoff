...
final class CatalogAdminService
{
    ...
    public function execute(string $command, array $payload): array
    {
        ...
        $map = [
            ...
            'assembly_component.remove'=>'removeAssemblyComponent'
        ];
    ...
    public function addAssemblyComponent(array $p): array {return $this->transaction(function()use($p){$aid=$this->requiredId($p,'assembly_catalog_item_id');$cid=$this->requiredId($p,'part_catalog_item_id');if($aid===$cid)throw new CatalogAdminException('ASSEMBLY_SELF_REFERENCE','Assembly cannot include itself.');$a=catalog_ra_assert_expected($this->pdo,'catalog_items',$aid,$p);$c=$this->tableRow('catalog_items',$cid);$this->assertUnlocked((int)$a['catalog_id']);if((int)$a['catalog_id']!==(int)$c['catalog_id'])throw new CatalogAdminException('ITEM_CATALOG_MISMATCH','Assembly component must belong to the same catalog.');$reverse=$this->pdo->prepare('SELECT 1 FROM assembly_parts WHERE assembly_catalog_item_id=? AND part_catalog_item_id=? AND deleted_at IS NULL LIMIT 1');$reverse->execute([$cid,$aid]);if($reverse->fetchColumn())throw new CatalogAdminException('ASSEMBLY_CYCLE','Assembly components cannot create a direct cycle.');$q=(float)($p['quantity']??0);if($q<=0)throw new CatalogAdminException('VALIDATION_ERROR','Quantity must be greater than zero.');$id=$this->insert('assembly_parts',['assembly_catalog_item_id'=>$aid,'part_catalog_item_id'=>$cid,'quantity'=>$q,'unit_cost_snapshot'=>(float)$c['unit_cost'],'unit_labor_time_snapshot'=>(float)$c['labor_hours']]);catalog_ra_created($this->pdo,'assembly_parts','assembly_component',$id,'assembly_component.created',$p,(int)$a['catalog_id']);$parent=$this->recalculateAssembly($aid,$p,'assembly.component_added');return ['id'=>$id,'entity'=>catalog_ra_row($this->pdo,'assembly_parts',$id),'assembly'=>$parent];});}
    public function updateAssemblyComponent(array $p): array {return $this->transaction(function()use($p){$id=$this->requiredId($p);$part=$this->tableRow('assembly_parts',$id);$aid=(int)$part['assembly_catalog_item_id'];$a=catalog_ra_assert_expected($this->pdo,'catalog_items',$aid,$p);$this->assertUnlocked((int)$a['catalog_id']);$q=(float)($p['quantity']??$part['quantity']);if($q<=0)throw new CatalogAdminException('VALIDATION_ERROR','Quantity must be greater than zero.');$componentInput=$p;unset($componentInput['expected_revision']);$entity=catalog_ra_update($this->pdo,'assembly_parts','assembly_component',$id,['quantity'=>$q],'assembly_component.updated',$componentInput,(int)$a['catalog_id']);$parent=$this->recalculateAssembly($aid,$p,'assembly.component_updated');return ['id'=>$id,'entity'=>$entity,'assembly'=>$parent];});}
